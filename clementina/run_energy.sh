#!/bin/bash
# Run distool (hybrid robust, reduced SOAP) over the sampled frames of one PKA
# energy and write a per-energy reconciliation CSV.
#   Args: LABEL  ENERGY_DIR  DUMP_PREFIX
# All distool side-outputs are contained in a node-local scratch dir (symlinks),
# so the source data directory is never written to.
set -u
LABEL=$1; DIR=$2; PREFIX=$3
BASE=$HOME/lammps-22Jul2025/build-clementina/pka_acero
DISTOOL=$HOME/distool_src/build/distool
OUTDIR=$HOME/distool_runs/out
BDDIR="$OUTDIR/breakdowns_${LABEL}"
mkdir -p "$OUTDIR" "$BDDIR"
REF="$BASE/$DIR/${PREFIX}.0"
...
ALL=$(ls "$BASE/$DIR/${PREFIX}."* 2>/dev/null | sed -E 's/.*\.([0-9]+)$/\1/' | grep -E '^[0-9]+$' | sort -n | uniq)

for s in $ALL; do
  [ "$s" = "0" ] && continue
  # sampling: keep every frame up to 20000 (ballistic+early relax), then every 5000
  ...
  # keep the small per-frame hybrid breakdown (signals + positions) for temporal ML
  if [ -f "$SCRATCH/hybrid_vacancies_out.csv" ]; then
    cp "$SCRATCH/hybrid_vacancies_out.csv" "$BDDIR/bd_${s}.csv"
  fi
  # clean side-outputs for next iteration
