#!/bin/bash
# Local adaptation of clementina/run_energy.sh.
# Run distool (hybrid robust, reduced SOAP) over the sampled ballistic frames of
# one PKA energy and write a per-energy WS-vs-hybrid reconciliation CSV.
#   Args: LABEL  ENERGY_DIR  DUMP_PREFIX
# All distool side-outputs (incl. the big analyzed_* dump) are kept inside a
# node-local scratch dir via symlinks, so /home/santi/pka_acero is never written.
set -u
